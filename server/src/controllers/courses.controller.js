...
async function assertTrainerExists(id) {
  ...
  const trainer = await db("users").where({ id, role: "TRAINER" }).first();
...
async function createCourse(req, res) {
  if (!required(req.body.title)) {
    return sendError(res, 400, "VALIDATION_ERROR", "Course title is required.");
  }

  const status = String(req.body.status || "DRAFT").toUpperCase();

  if (!COURSE_STATUSES.includes(status)) {
    return sendError(res, 400, "VALIDATION_ERROR", "Invalid course status.");
  }
...
async function listCourses(req, res) {
  const query = db("courses as c")
    ...
    .select("c.*", "co.name as cohort_name", "t.username as trainer_name")
  ...
  if (req.query.status) {
    query.where("c.status", String(req.query.status).toUpperCase());
  }
...
async function getCourseById(req, res) {
  ...
  if (!canReadCourse(req.user, course)) {
    return sendError(res, 403, "FORBIDDEN", "You cannot access this course.");
  }
